  ...
  render() {
    ...
    const newscontainer = (
      <div>
        ...
        <div>
          ARTICLE URL{" "}
          <a href={isLoaded ? polygonData.results[0].article_url : "loading"}>
            {isLoaded ? polygonData.results[0].article_url : "loading"}
          </a>
        </div>
        <div>
          ARTICLE DESCRIPTION{" "}
          {isLoaded ? polygonData.results[0].description : "loading"}
        </div>
        <div>BREAK BREAK BREAK</div>
        <div>TITLE: {isLoaded ? polygonData.results[1].title : "loading"}</div>
        <div>
          ARTICLE URL{" "}
          <a href={isLoaded ? polygonData.results[1].article_url : "loading"}>
            {isLoaded ? polygonData.results[1].article_url : "loading"}
          </a>
        </div>
        <div>
          ARTICLE DESCRIPTION{" "}
          {isLoaded ? polygonData.results[1].description : "loading"}
        </div>
        <div>BREAK BREAK BREAK</div>
        <div>TITLE: {isLoaded ? polygonData.results[2].title : "loading"}</div>
        <div>
          ARTICLE URL{" "}
          <a href={isLoaded ? polygonData.results[2].article_url : "loading"}>
            {isLoaded ? polygonData.results[2].article_url : "loading"}
          </a>
        </div>
        <div>
          ARTICLE DESCRIPTION{" "}
          {isLoaded ? polygonData.results[2].description : "loading"}
        </div>
        <div>BREAK BREAK BREAK</div>
      </div>
    );
    ...
    return (
      <div>
        <div>
          <button onClick={() => this.apifetcher()}>
            TEST THE POLYGON API{" "}
          </button>
        </div>
        <div>
